            ...
            faceAuth.initLicenseOffLine(context, new Callback() {
                ...
                public void onResponse(int code, String response) {
                    if (code == 0) {
                        ...
                        if (listener != null) {
                            listener.initFail(code, response);
                        }
                    }
                }
            });
    ...
    public void onDetectCheck(final byte[] rgbData,
        ...
        future = es.submit(new Runnable() {
            ...
            public void run() {
                ...
                faceInfos = FaceSDKManager.getInstance().getFaceDetect().track(BDFaceSDKCommon.DetectType.DETECT_VIS, rgbInstance);
                ...
                //boolean faceLandmarks = faceSizeFilter(faceInfos[0],srcWidth,srcHeight);
                // 检测结果判断
                if (faceInfos != null && faceInfos.length > 0/* && faceLandmarks == true*/) {
...
//                Log.e("TAG", "onFeatureCheck: " + topFeature.getScore());
     ...
     * @param faceFeatureCallBack 回调方法
     */
